Page({
  ...
  onLoad() { this.setData({ filteredServices: this.data.services }) },
  onSearch(e) {
    const kw = e.detail.value.toLowerCase()
    const filtered = kw ? this.data.services.filter(s => s.name.toLowerCase().includes(kw) || s.desc.toLowerCase().includes(kw)) : this.data.services
    this.setData({ keyword: kw, filteredServices: filtered })
  },
